...
def slide_window(img, x_start_stop=[None, None], y_start_stop=[None, None],
    ...
    # we'll be considering windows one by one with your
...
def find_cars(img, ystart, ystop, xstart, xstop, scale,
            ...
            spatial_size, hist_bins, cells_per_step, visualize = False):
    boxes = []
    ...
    if scale != 1:
        ...
        dims = (np.int(imshape[1]/scale), np.int(imshape[0]/scale))
        ctrans_tosearch = cv2.resize(ctrans_tosearch, dims)
    ...
    """
    Previously we extracted HOG features from each individual window as we
    searched across the image, but it turns out this is rather inefficient. To
    speed things up, extract HOG features just once for the entire region of
    interest (i.e. lower half of each frame of video) and subsample that array
    for each sliding window. This is done by feature_vec=False
    """
